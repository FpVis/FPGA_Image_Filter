...
module i2c_ov7670_unit (
    ...
    VGA_I2C_ControlUnit U_VGA_I2C_ControlUnit (
        .clk      (clk),
        .reset    (reset),
        .start    (start),
        .reg_addr (reg_addr),
    ...
    i2c_ov7670 U_i2c_ov7670 (
        .clk      (clk),       // 시스템 클럭
        .reset    (reset),     // 리셋 신호
        .start    (start),     // 전송 시작 신호
        .reg_addr (reg_addr),  // 레지스터 주소
        .data     (data),      // 레지스터 데이터
        .scl      (scl),       // I2C 클럭
        .sda      (sda),       // I2C 데이터 라인 (양방향)
        .done     (done),
        .ack_error(ack_error)  // 전송 완료 신호  // ACK 오류 신호
    );
...
module i2c_ov7670 (
    ...
    output logic       done,
    output logic       ack_error  // 전송 완료 신호  // ACK 오류 신호
);

    // 상태 정의
    logic [3:0] bit_cnt_reg, bit_cnt_next;  // 비트 카운터
    logic [9:0] clk_cnt_reg, clk_cnt_next;
    logic sda_out_reg, sda_out_next;  // SDA 출력
    logic sda_en_reg, sda_en_next;  // SDA 활성화
    logic [7:0] shift_reg_reg, shift_reg_next;  // 전송 데이터 버퍼
    logic
        scl_mode_reg,
        scl_mode_next,
        ack_error_reg,
    ...
    always_ff @(posedge clk, posedge reset) begin : continuous_circuit
        if (reset) begin
            state             <= IDLE;
            bit_cnt_reg       <= 7;
            clk_cnt_reg       <= 0;
    ...
    always_comb begin : combinational_circuit
        state_next         = state;
        ...
        case (state)
            IDLE: begin
                ...
                if(start) begin // Start가 들어오면 sda 는 1 유지 scl 도 1 유지
                    sda_en_next  = 1'b1;
                    sda_out_next = 1'b1;
                    state_next   = START_1;
                end
...
module VGA_I2C_ControlUnit (
    ...
    localparam RESET = 0, SET_QVGA_RGB = 1, WAIT = 2, RUN = 3, DATA = 4;
    ...
    initial begin
        ...
        addr_dat[74] = 16'h13_e5; // COM8. Fast AGC/AEC 알고리즘 활성화, AEC step size 무제한, AGC 활성화, AWB 비활성화, AEC 활성화, banding filter OFF
